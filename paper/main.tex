...
$12.8$, and in a finite population the unsafe attractor survives between
$10^{2}$ and $10^{13}$ generations. Fourth, coupling the dynamics to a
non-decreasing stock of diffused capability that erodes enforcement produces
hysteresis whose width is exactly computable and independent of the speed of
diffusion. The results locate the governance problem at the selection layer
rather than at the design layer, and identify liability regimes that are worse
than either extreme.
\end{abstract}
...
\item Selection of AI designs by principals defines a payoff wedge that drives the dynamics.
\item Solo safety evaluations remove a weakly dominated design and cannot move equilibrium.
\item Conditional safety is 78 times cheaper to protect than unconditional safety.
\item Long-run harm is non-monotone: a wide window of liability makes outcomes worse.
\item Irreversible capability diffusion turns the transition into a hysteresis loop.
\end{itemize}
...
numerically. Section~\ref{sec:model} specifies the two layers and
Section~\ref{sec:numerics} the numerical protocol,
Section~\ref{sec:reduction} assembles the closed forms,
Section~\ref{sec:finite} moves to finite populations,
Section~\ref{sec:robust} tests the thresholds against the constants of the
race, and Section~\ref{sec:discussion} draws out what the results imply for
governance. The four results are:
...
return to that in Section~\ref{sec:discussion}.
...
Table~\ref{tab:payoff} reports $a$ and Table~\ref{tab:unsafe-count} reports
$m$; both are reproduced with the threshold table in Appendix~\ref{app:tables}.
...
Every number quoted in the text is regenerated by the released code.
...
Section~\ref{sec:face}, so its protection lasts only as long as no
$\CAS$-like design re-enters, and Theorem~\ref{thm:barrier} bounds how much
liability is needed when one does. Second, the strict threshold is only
achievable because the probe environment in \eqref{eq:solo} is fully safe:
measured against an unsafe opponent the conditionally safe design scores
$u(\CS,\CAS)=0.461$ and would be rejected by the same rule. A filter strict
enough to exclude conditional aggression in a mixed environment also excludes
conditional safety, which Theorem~\ref{thm:barrier} identifies as the cheapest
design to protect.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{figures/fig04_bifurcation.pdf}
...
%======================================================================
\section{Robustness to the constants of the race}\label{sec:robust}
%======================================================================
...
grows with the prize, from a factor of $4.4$ at $B=10$ to $90$ at $B=400$ for
$p_r^{\max}=0.6$. At $p_r^{\max}=0.9$ the comparison degenerates in the
...
\label{eq:coupled}
\end{equation}
with $\varepsilon\ll 1$ separating the time scales and $L_{\mathrm{eff}}$ an
erosion channel. The construction differs from renewable game--environment
models~\citep{weitz2016oscillating,tilman2020feedbacks} in that
$\dot z\ge 0$ identically: the environmental state is a ratchet.

\begin{theorem}[Hysteresis width]\label{thm:hysteresis}
Assume the safe attractor has $U=0$ exactly, as in
Proposition~\ref{prop:twins}, and consider a quasi-static sweep of $L$ that
starts in the protected regime. The protected regime is lost at
$L_{\downarrow}=L^{*}_{\CAS\to\CS}$, after which $z\to 1$; it is recovered on
the return sweep at $L_{\uparrow}=L^{*}_{\CAS\to\CS}/\rho$, where
$\rho=L_{\mathrm{eff}}(1)/L$ is the residual fraction of enforcement at full
...
\label{eq:width}
\end{equation}
independently of the diffusion rate $\varepsilon$.
\end{theorem}

\begin{proof}
On the protected branch $U(x)=0$, so the second equation of \eqref{eq:coupled}
gives $\dot z=0$ and $L_{\mathrm{eff}}=L$; the branch is lost exactly when
$\CAS$ can invade the resident safe face, i.e.\ at $L=L^{*}_{\CAS\to\CS}$ by
Lemma~\ref{lem:affine} and \eqref{eq:invasion}. On the unsafe branch $U>0$, so
$z\to1$ and $L_{\mathrm{eff}}\to\rho L$; recovery requires
$L_{\mathrm{eff}}>L^{*}_{\CAS\to\CS}$, which gives \eqref{eq:width}. Neither
step uses $\varepsilon$.
\end{proof}

The non-trivial ingredient is not the erosion channel but
Proposition~\ref{prop:twins}: because the protected branch has $U=0$
\emph{exactly}, the ratchet variable is frozen there, so the loss threshold is
the bare invasion threshold and the width depends on the channel only through
the single number $\rho$. Two different channels with the same residual
fraction give the same width, as \eqref{eq:width} shows for $\theta=0.9$ and
$\kappa=9$.
...
at the baseline and up to $90$ at large prizes. Conditional safety is cheap to
...
whole range of prizes and risk levels we examined. The distinction between
initiating and retaliating is available in principle -- it is a matter of
conditioning the rule on the counterparty's prior action, which is exactly the
column-constant structure that Proposition~\ref{prop:inert} identifies as
inert. Designing an implementable approximation of that rule is the natural
next question.

\paragraph{Structured populations.}
The analysis is well mixed. Network reciprocity generally favours
conditionally cooperative strategies by clustering them~\citep{ohtsuki2006simple,santos2006structured}, and we expect it to act
unevenly on our four mechanisms. The weak dominance of $\CAS$ over $\AU$
(Theorem~\ref{thm:dominance}) is a pointwise payoff comparison and survives
any matching structure. The neutrality of the $\AS$--$\CS$ face
(Proposition~\ref{prop:twins}) is likewise a statement about identical payoffs
and survives, although the drift dynamics on it does not. By contrast the
valley depends on the balance between two invasion conditions and should
narrow under assortment, since clustering raises the payoff a conditionally
safe design obtains from its own kind without raising the retaliation it must
pay for. Establishing whether it closes is left open.
...
than derived; Theorem~\ref{thm:hysteresis} isolates what does not depend on
that assumption, but the assumption of erosion itself remains. Finally, the
analysis is deterministic in $h$ and $\lambda$, whereas real liability arrives
with probability and delay, so an expected-value reading of $L$ is a first
approximation only.
...
population for up to $10^{13}$ generations. And irreversible capability
