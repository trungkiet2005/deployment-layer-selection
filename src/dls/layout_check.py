...
def _offscreen_tick_labels(fig: plt.Figure) -> set[int]:
    ...
    An axes switched off with ``ax.axis("off")`` is the same problem in a
    different guise: the axis is not drawn, but its ticks and its axis label
    keep ``visible`` set and keep reporting extents, so every one of them is
    dropped here.
    """
    skip: set[int] = set()
...
def texts_over_data(
    ...
    A twin axis occupies the same panel, so its curves lie under the same
    text; they are gathered here rather than left to the twin's own turn,
    which would miss them because the legend belongs to the primary axes.
    """
    fig.canvas.draw()
    renderer = fig.canvas.get_renderer()
    skip = _offscreen_tick_labels(fig)
    ...
    for panel, ax in enumerate(all_axes):
        ...
        panel_axes = [other for other in all_axes
                      if other is ax or other.bbox.bounds == ax.bbox.bounds]
        lines = [(source, line) for source in panel_axes
        ...
        for source, line in lines:
            ...
            if data is None or len(data) == 0:
                continue
            pixels = source.transData.transform(data)
