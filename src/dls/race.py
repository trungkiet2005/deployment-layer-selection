...
The four reduced strategies (AS, AU, CS, CAS) are deterministic, so the whole
action path of an ordered pair is fixed once the pair is fixed.  The only
stochastic primitive is the horizon ``T``.  We therefore evaluate every matchup
by exact expectation over the horizon distribution rather than by Monte Carlo
sampling, which removes simulation noise from the payoff matrix entirely.
"""
...
#: First-round action and conditionality of each reduced strategy.
...
class RaceParams:
    ...
    p_max:
        Treatment-level maximum private setback risk ``p_r^max``.
    setback_scope:
    ...
    def horizon_distribution(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        ``T = min_rounds + G`` with ``G ~ Geom(stop_prob) - 1`` on
        ``{0, 1, 2, ...}``.  The truncated tail mass is assigned to
        ``max_rounds`` so that the probabilities sum to one exactly.
        """
        t_values = np.arange(self.min_rounds, self.max_rounds + 1)
        g = t_values - self.min_rounds
        probs = self.stop_prob * (1.0 - self.stop_prob) ** g
        # assign the truncated tail to the last atom, guarding against the
        # rounding error that makes the head sum to slightly more than one
...
class MatchupOutcome:
    """Horizon-averaged quantities for one ordered strategy pair."""

    payoff: float
    """Expected task payoff of the focal player, ``pi_A``."""

    unsafe_count: float
    """Expected number of Unsafe actions of the focal player, ``E[n^U]``."""

    unsafe_frequency: float
    """Expected fraction of Unsafe rounds of the focal player, ``E[n^U / T]``."""

    win_probability: float
    """Probability that the focal player strictly leads at the horizon."""

    tie_probability: float
    """Probability of equal progress at the horizon."""

    setback_probability: float
    """Expected realised setback probability, ``E[1{win or tie} q]``."""


def evaluate_matchup(row: str, col: str, params: RaceParams) -> MatchupOutcome:
    """Evaluate one ordered strategy pair exactly over the horizon law."""
...
class RaceTables:
    ...
    Every matrix is indexed ``[i, j]`` = focal strategy ``i`` against
    opponent strategy ``j``, with the strategy order of :data:`STRATEGIES`.
    """
    ...
    def subset(self, strategies: tuple[str, ...]) -> "RaceTables":
        """Restrict all matrices to a sub-pool of designs."""
        idx = [self.strategies.index(s) for s in strategies]
        sel = np.ix_(idx, idx)
        return RaceTables(
            strategies=tuple(strategies),
            payoff=self.payoff[sel],
            unsafe_count=self.unsafe_count[sel],
            unsafe_frequency=self.unsafe_frequency[sel],
            win_probability=self.win_probability[sel],
            tie_probability=self.tie_probability[sel],
            setback_probability=self.setback_probability[sel],
            params=self.params,
        )
...
def build_race_tables(
    ...
    """Evaluate every ordered strategy pair of the reduced race game."""
    n = len(strategies)
    payoff = np.zeros((n, n))
